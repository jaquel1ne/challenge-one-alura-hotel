...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
	    super("Reserva");
	    try {
	        this.reservaController = new ReservaController(new ReservaDAO(ConnectionFactory.getConnection()));
		...
		txtDataE.addPropertyChangeListener("date", new PropertyChangeListener() {
		    public void propertyChange(PropertyChangeEvent evt) {
		        calcularValorReserva();
		    }
		});
		
		txtDataS.addPropertyChangeListener("date", new PropertyChangeListener() {
		    public void propertyChange(PropertyChangeEvent evt) {
		        calcularValorReserva();
		    }
		});
		...
		btnProximo.addMouseListener(new MouseAdapter() {
		    ...
		    public void mouseClicked(MouseEvent e) {
		        if (txtDataE.getDate() != null && txtDataS.getDate() != null) {
		            ...
		            if (!"00,00".equals(valorStr)) {
		                Reserva reserva = new Reserva();
		                reserva.setDataEntrada(txtDataE.getDate());
		                reserva.setDataSaida(txtDataS.getDate());
		                FormaPagamento formaPagamento = FormaPagamento.getByDescricao(txtFormaPagamento.getSelectedItem().toString());
		                ...
		                try {
		                    ...
		                    ex.printStackTrace();
		                    JOptionPane.showMessageDialog(null, "Erro ao converter o valor para o formato correto.");
		                    return;
		                }

		                Long idReserva = reservaController.salvar(reserva);
		                if (idReserva != null) {
		                    reserva.setId(idReserva); 
		                    
		                    RegistroHospede registro = new RegistroHospede(reserva);
		                    registro.setupInitialData();
		                    registro.setVisible(true);
		                    dispose();
		                } else {
		                    JOptionPane.showMessageDialog(null, "Erro ao salvar a reserva. Tente novamente.");
		                }
		            } else {
		                JOptionPane.showMessageDialog(null, "O valor da reserva não foi corretamente calculado. Por favor, tente novamente.");
		            }
		        } else {
	...
	private void calcularValorReserva() {
	    if (txtDataE.getDate() != null && txtDataS.getDate() != null) {
	        long diferencaMilissegundos = txtDataS.getDate().getTime() - txtDataE.getDate().getTime();
	        int dias = (int) (diferencaMilissegundos / (1000 * 60 * 60 * 24));
	        ...
	        double taxaDiaria = 100.0;
	        double valorTotal = dias * taxaDiaria;
