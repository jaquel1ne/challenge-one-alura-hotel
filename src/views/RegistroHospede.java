...
public class RegistroHospede extends JFrame {
	...
	public RegistroHospede(Reserva reserva) {
		try {
	        ...
	        this.hospedeController = new HospedeController(new HospedeDAO(ConnectionFactory.getConnection()));
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
		...
		btnsalvar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				 if (reservaAtual == null) {
				        System.out.println("Erro: reservaAtual é null!");
				        return;
				    }
				    
				    Long idReserva = reservaController.salvar(reservaAtual);
				    ...
				    if(idReserva != null) {
			            ...
			            JOptionPane.showMessageDialog(null, "Erro ao salvar a reserva. Por favor, tente novamente.");
			        }
				    
				    Hospede novoHospede = new Hospede();
				    novoHospede.setNome(txtNome.getText());
				    novoHospede.setSobrenome(txtSobrenome.getText());
				    novoHospede.setDataNascimento(txtDataN.getDate());
				    novoHospede.setNacionalidade((String) txtNacionalidade.getSelectedItem());
				    novoHospede.setTelefone(txtTelefone.getText());
				    novoHospede.setIdReserva(idReserva); 
	...
	public void setupInitialData() {
	    if (reservaAtual != null && reservaAtual.getId() != null) {
	        txtNreserva.setText(reservaAtual.getId().toString());
	        txtNreserva.setEditable(false);
	    }
	}
